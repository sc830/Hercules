      ...
      <ScrollView>
        <View style={{ alignItems: 'center' }}>
          <TouchableOpacity onPress={handleAdd} style={{ backgroundColor: 'purple', padding: 20, margin: 20, width: '80%' }}>
            <Text style={{ color: 'white', textAlign: 'center' }}>Add Water Intake</Text>
          </TouchableOpacity>
        </View>
        {itemList.map((trackedItem, index) => (
          <TouchableOpacity
            key={index}
            ...
            onPress={() => handleEdit(trackedItem)}
          >
            <Text style={{ color: 'black' }}>{trackedItem}</Text>
      ...
      <Modal animationType="slide" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
          <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10, width: '80%' }}>
            ...
            {selectedItem === null && (
              <View>
                <Text>How many ounces of water have you drank:</Text>
                <TextInput
                  style={{ borderWidth: 1, borderColor: 'black', borderRadius: 5, padding: 5 }}
                  value={trackedItem}
                  onChangeText={(text) => setItem(text)}
